...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.psch", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for psch issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "psch-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/seaskycheng/pschtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for psch and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.psch", "github.com/petercastron/PSCH/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "psch.aar"), "psch.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "psch", "pschtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
