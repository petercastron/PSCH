...
// Package core implements the psch consensus protocol.
package core
...
import (
	...
	"github.com/petercastron/PSCH/common/prque"
...
// available in the database. It initialises the default psch Validator and
// Processor.
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. psch. is initialized with
...
// sync, psch. will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init psch. with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded psch. to a new version which supports transaction
