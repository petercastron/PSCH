...
import (
	...
	"github.com/petercastron/PSCH/crypto/signify"
	"github.com/petercastron/PSCH/internal/build"
	"github.com/petercastron/PSCH/params"
)

var (
	// Files that end up in the psch*.zip archive.
	pschArchiveFiles = []string{
		"COPYING",
		executablePath("psch"),
	}

	// Files that end up in the psch-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert psch contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "psch bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (psch Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "psch",
			Description: "psch CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "psch private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "psch account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "pschstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basepsch = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		psch     = "psch-" + basepsch + ext
		alltools = "psch-alltools-" + basepsch + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(psch, pschArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{psch, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with psch-release.pub"
...
func doDockerImage(cmdline []string) {
	var (
		upload = flag.String("upload", "", `Where to upload the docker image (usually "psch/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - psch/client-go:latest                            - Pushes to the master branch, psch only
	//  - psch/client-go:stable                            - Version tag publish on GitHub, psch only
	//  - psch/client-go:alltools-latest                   - Pushes to the master branch, psch & tools
	//  - psch/client-go:alltools-stable                   - Version tag publish on GitHub, psch & tools
	//  - psch/client-go:release-<major>.<minor>           - Version tag publish on GitHub, psch only
	//  - psch/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, psch & tools
	//  - psch/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, psch only
	//  - psch/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, psch & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "seaskycheng/psch")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "psch-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the psch sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "psch-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "psch"
...
type debMetadata struct {
	...
	// psch version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "psch Builds <418012770@qq.com>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "psch.exe" {
			pschTool = file
		} else {
	...
	// first section contains the psch binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"psch":     pschTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.psch.nsi", filepath.Join(*workdir, "psch.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("psch-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "psch.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "pschstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.psch", "-v", "github.com/petercastron/PSCH/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("psch.aar", filepath.Join(GOBIN, "psch.aar"))
		os.Rename("psch-sources.jar", filepath.Join(GOBIN, "psch-sources.jar"))
		return
	}
	...
	archive := "psch-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("psch.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/petercastron/PSCH/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "psch-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "psch.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "psch.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "pschstore/builds")`)
