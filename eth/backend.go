...
// Package eth implements the psch protocol.
package eth
...
import (
	...
	"github.com/petercastron/PSCH/core/state/pruner"
	...
	"github.com/petercastron/PSCH/eth/protocols/eth"
	"github.com/petercastron/PSCH/eth/protocols/snap"
	...
	"github.com/petercastron/PSCH/p2p/dnsdisc"
...
// Ethereum implements the psch full node service.
...
// New creates a new psch object (including the
// initialisation of the common psch object)
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// Assemble the psch object
	chainDb, err := stack.OpenDatabaseWithFreezer("chaindata", config.DatabaseCache, config.DatabaseHandles, config.DatabaseFreezer, "psch/db/chaindata/", false)
	if err != nil {
		return nil, err
	}
	...
	log.Info("Initialising psch protocol", "network", config.NetworkId, "dbversion", dbVer)

	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, psch %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
...
// APIs return the collection of RPC services the psch package offers.
