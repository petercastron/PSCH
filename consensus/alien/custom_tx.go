...
const (
	...
	categoryExch       = "Exch"
	categoryMultiSign  = "Multi"
	categoryBind       = "Bind"
	categoryUnbind     = "Unbind"
	categoryRebind     = "Rebind"
	categoryCandReq    = "CandReq"
	categoryCandExit   = "CandExit"
	categoryCandPnsh   = "CandPnsh"
	categoryFlwReq     = "FlwReq"
	categoryFlwExit    = "FlwExit"
	...
	for _, tx := range txs {
		...
		if len(string(tx.Data())) >= len(ufoPrefix) {
			...
			if len(txDataInfo) >= ufoMinSplitLen {
				...
				} else if txDataInfo[posPrefix] == phPrefix {
					if txDataInfo[posVersion] == ufoVersion {
						if txDataInfo[posCategory] == categoryExch {
						...
						} else if txDataInfo[posCategory] == categoryRebind {
						...
						} else if txDataInfo[posCategory] == categoryCandReq {
						...
						} else if txDataInfo[posCategory] == categoryCandExit {
						...
						} else if txDataInfo[posCategory] == categoryCandPnsh {
						...
						} else if txDataInfo[posCategory] == categoryFlwReq {
						...
						} else if txDataInfo[posCategory] == categoryFlwExit {
...
func (a *Alien) processCreateMultiSignature (txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB) {
	if len(txDataInfo) <= posThreshold + 2 {
	...
	if threshold, err := strconv.ParseUint(txDataInfo[posThreshold], 10, 32); err == nil {
		if 2 > threshold || 10 < threshold {
			log.Warn("Create Multi-Signature", "threshold", txDataInfo[posThreshold])
			return
		} else {
			if len(txDataInfo) < posThreshold + 2 + int(threshold) || len(txDataInfo) > posThreshold + 1000 {
				log.Warn("Create Multi-Signature fail", "parameter number", len(txDataInfo))
				return
			}
		}
...
func (a *Alien) processExchangeNFC (currentExchangeNFC []ExchangeNFCRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []ExchangeNFCRecord {
	if len(txDataInfo) <= posExchValue {
	...
	if err := exchangeNFC.Target.UnmarshalText1([]byte(txDataInfo[posExchAddress])); err != nil {
		log.Warn("Exchange NFC to FUL fail", "address", txDataInfo[posExchAddress])
		return currentExchangeNFC
	}
	amount := big.NewInt(0)
	var err error
	if amount, err = hexutil.UnmarshalText1([]byte(txDataInfo[posExchValue])); err != nil {
		log.Warn("Exchange NFC to FUL fail", "number", txDataInfo[posExchValue])
		return currentExchangeNFC
	}
...
func (a *Alien) processDeviceBind(currentDeviceBind []DeviceBindRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, snap *Snapshot, number uint64) []DeviceBindRecord {
	if len(txDataInfo) <= posMiltiSign {
	...
	if err := deviceBind.Device.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Device bind revenue", "miner address", txDataInfo[posMinerAddress])
		return currentDeviceBind
	}
	if revenueType, err := strconv.ParseUint(txDataInfo[posRevenueType], 10, 32); err == nil {
		if revenueType == 0 {
			if _, ok := snap.RevenueNormal[deviceBind.Device]; ok {
				log.Warn("Device bind revenue", "device already bond", txDataInfo[posMinerAddress])
				return currentDeviceBind
			}
		} else {
			if number >=StorageEffectBlockNumber {
		...
		log.Warn("Device bind revenue", "type", txDataInfo[posRevenueType])
		return currentDeviceBind
	}
	if number<PledgeRevertLockEffectNumber{
		if 0 < len(txDataInfo[posRevenueContract]) {
			if err := deviceBind.Contract.UnmarshalText1([]byte(txDataInfo[posRevenueContract])); err != nil {
				log.Warn("Device bind revenue", "contract address", txDataInfo[posRevenueContract])
				return currentDeviceBind
			}
		}
		if 0 < len(txDataInfo[posMiltiSign]) {
			if err := deviceBind.MultiSign.UnmarshalText1([]byte(txDataInfo[posMiltiSign])); err != nil {
				log.Warn("Device bind revenue", "milti-signature address", txDataInfo[posRevenueContract])
				return currentDeviceBind
			}
		}
	}
	if number >=StorageEffectBlockNumber {
		if len(txDataInfo) > posRevenueAddress {
			if 0 < len(txDataInfo[posRevenueAddress]) {
				if err := deviceBind.Revenue.UnmarshalText1([]byte(txDataInfo[posRevenueAddress])); err != nil {
					log.Warn("Device bind revenue", "Revenue address", txDataInfo[posRevenueAddress])
					return currentDeviceBind
				}
			}
		}
...
func (a *Alien) processDeviceUnbind(currentDeviceBind []DeviceBindRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot, number uint64) []DeviceBindRecord {
	if len(txDataInfo) <= posRevenueType {
	...
	if err := deviceBind.Device.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Device unbind revenue", "miner address", txDataInfo[posMinerAddress])
		return currentDeviceBind
	}
	if revenueType, err := strconv.ParseUint(txDataInfo[posRevenueType], 10, 32); err == nil {
		if revenueType == 0 {
			if oldBind, ok := snap.RevenueNormal[deviceBind.Device]; !ok {
				log.Warn("Device unbind revenue", "device never bond", txDataInfo[posMinerAddress])
				return currentDeviceBind
			} else {
		...
		log.Warn("Device unbind revenue", "type", txDataInfo[posRevenueType])
		return currentDeviceBind
	}
	topics := make([]common.Hash, 3)
	topics[0].UnmarshalText([]byte("0xf061654231b0035280bd8dd06084a38aa871445d0b7311be8cc2605c5672a6e3")) //web3.sha3("DeviceBind(uint32,byte32,byte32,address)")
...
func (a *Alien) processDeviceRebind(currentDeviceBind []DeviceBindRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot, number uint64) []DeviceBindRecord {
	...
	if err := deviceBind.Device.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Device rebind revenue", "miner address", txDataInfo[posMinerAddress])
		return currentDeviceBind
	}
	if err := deviceBind.Revenue.UnmarshalText1([]byte(txDataInfo[posRevenueAddress])); err != nil {
		log.Warn("Device rebind revenue", "revenue address", txDataInfo[posMinerAddress])
		return currentDeviceBind
	}
	if revenueType, err := strconv.ParseUint(txDataInfo[posRevenueType], 10, 32); err == nil {
		if revenueType == 0 {
		...
		log.Warn("Device rebind revenue", "type", txDataInfo[posRevenueType])
		return currentDeviceBind
	}
	if number<PledgeRevertLockEffectNumber{
		if 0 < len(txDataInfo[posRevenueContract]) {
			if err := deviceBind.Contract.UnmarshalText1([]byte(txDataInfo[posRevenueContract])); err != nil {
				log.Warn("Device rebind revenue", "contract address", txDataInfo[posRevenueContract])
				return currentDeviceBind
			}
		}
		if 0 < len(txDataInfo[posMiltiSign]) {
			if err := deviceBind.MultiSign.UnmarshalText1([]byte(txDataInfo[posMiltiSign])); err != nil {
				log.Warn("Device rebind revenue", "milti-signature address", txDataInfo[posRevenueContract])
				return currentDeviceBind
			}
		}
	}
...
func (a *Alien) processCandidatePledge (currentCandidatePledge []CandidatePledgeRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []CandidatePledgeRecord {
	if len(txDataInfo) <= posMinerAddress {
	...
	if err := candidatePledge.Target.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Candidate pledge", "miner address", txDataInfo[posMinerAddress])
		return currentCandidatePledge
	}
...
func (a *Alien) processCandidateExit (currentCandidateExit []common.Address, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []common.Address {
	...
	nilHash := common.Address{}
	zeroHash := common.BigToAddress(big.NewInt(0))
	if err := minerAddress.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Candidate exit", "miner address", txDataInfo[posMinerAddress])
		return currentCandidateExit
	}
...
func (a *Alien) processCandidatePunish (currentCandidatePunish []CandidatePunishRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []CandidatePunishRecord {
	...
	if err := candidatePunish.Target.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Candidate punish", "miner address", txDataInfo[posMinerAddress])
		return currentCandidatePunish
	}
...
func (a *Alien) processMinerPledge (currentClaimedBandwidth []ClaimedBandwidthRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []ClaimedBandwidthRecord {
	if len(txDataInfo) <= posBandwidth {
	...
	if err := claimedBandwidth.Target.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Claimed bandwidth", "miner address", txDataInfo[posMinerAddress])
		return currentClaimedBandwidth
	}
	...
	if ISPQosID, err := strconv.ParseUint(txDataInfo[posISPQosID], 16, 32); err != nil {
		log.Warn("Claimed bandwidth", "ISP qos id", txDataInfo[posISPQosID])
		return currentClaimedBandwidth
	} else {
		claimedBandwidth.ISPQosID = uint32(ISPQosID)
	}
	if bandwidth, err := strconv.ParseUint(txDataInfo[posBandwidth], 16, 32); err != nil {
		log.Warn("Claimed bandwidth", "bandwidth", txDataInfo[posBandwidth])
		return currentClaimedBandwidth
	} else {
...
func (a *Alien) processMinerExit (currentFlowMinerExit []common.Address, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, state *state.StateDB, snap *Snapshot) []common.Address {
	...
	zeroHash := common.BigToAddress(big.NewInt(0))
	if err := minerAddress.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Flow miner exit", "miner address", txDataInfo[posMinerAddress])
		return currentFlowMinerExit
	}
...
func (a *Alien) processBandwidthPunish (currentBandwidthPunish []BandwidthPunishRecord, txDataInfo []string, txSender common.Address, tx *types.Transaction, receipts []*types.Receipt, snap *Snapshot) []BandwidthPunishRecord {
	...
	if err := bandwidthPunish.Target.UnmarshalText1([]byte(txDataInfo[posMinerAddress])); err != nil {
		log.Warn("Bandwidth punish", "miner address", txDataInfo[posMinerAddress])
		return currentBandwidthPunish
	}
