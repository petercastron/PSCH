...
// psch is the official command-line client for psch.
package main
...
import (
	...
	"github.com/petercastron/PSCH/console/prompt"
...
const (
	clientIdentifier = "psch" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the psch command line interface")
...
func init() {
	// Initialize the CLI app and start psch
	app.Action = psch
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2021 The psch Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.TestnetFlag.Name):
		log.Info("Starting psch on testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting psch in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting psch on psch mainnet...")
	}
...
// psch is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func psch(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	// Create a client to interact with local psch node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full psch node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("psch service not running: %v", err)
		}
