...
// nodeDockerfile is the Dockerfile required to run an psch node.
var nodeDockerfile = `
FROM psch/client-go:latest
...
{{end}}
RUN \
  echo 'psch --cache 512 init /genesis.json' > psch.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.psch/keystore/ && cp /signer.json /root/.psch/keystore/' >> psch.sh && \{{end}}
	echo $'exec psch --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> psch.sh

ENTRYPOINT ["/bin/sh", "psch.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an psch node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.psch{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.pschhash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new psch node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 psch --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.psch"],
		ethashdir:  infos.volumes["/root/.pschhash"],
		port:       port,
