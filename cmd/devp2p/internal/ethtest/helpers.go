...
import (
	...
	"github.com/petercastron/PSCH/internal/utesting"
	"github.com/petercastron/PSCH/p2p"
	"github.com/petercastron/PSCH/p2p/rlpx"
)
...
func (s *Suite) dial() (*Conn, error) {
	...
	conn.caps = []p2p.Cap{
		{Name: "psch", Version: 64},
		{Name: "psch", Version: 65},
	}
...
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "psch", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected psch protocol version")
		}
		return nil
	default:
...
// negotiatepschProtocol sets the Conn's psch protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "psch" {
			continue
		}
...
loop:
	...
	// make sure psch protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("psch protocol version must be set in Conn")
	}
	if status == nil {
