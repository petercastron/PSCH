...
Use `devp2p enrdump <base64>` to verify and display an psch Node Record.
...
- `-psch-network <mainnet/rinkeby/goerli/ropsten>` filters nodes by "psch" ENR entry
- `-les-server` filters nodes by LES server support
- `-snap` filters nodes by snap protocol support

For example, given a node set in `nodes.json`, you could create a filtered set containing
up to 20 psch mainnet nodes which also support snap sync using this command:

    devp2p nodeset filter nodes.json -psch-network mainnet -snap -limit 20
...
### psch Protocol Test Suite

The psch Protocol test suite is a conformance test suite for the [psch protocol][psch].

To run the psch protocol test suite against your implementation, the node needs to be initialized as such:

1. initialize the psch node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run psch with the following flags:
```
psch --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the psch node: 
 ```
 devp2p rlpx psch-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```
...
To run the eth66 protocol test suite, initialize a psch node as described above and run the following command,
replacing `<enode>` with the enode of the psch node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[psch]: https://github.com/seaskycheng/devp2p/blob/master/caps/psch.md
[dns-tutorial]: https://psch.org/docs/developers/dns-discovery-setup
