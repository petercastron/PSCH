...
import (
	...
	"github.com/petercastron/PSCH/accounts/scwallet"
	"github.com/petercastron/PSCH/common"
	"github.com/petercastron/PSCH/common/hexutil"
	"github.com/petercastron/PSCH/common/math"
	...
	"github.com/petercastron/PSCH/consensus/misc"
...
// PublicpschAPI provides an API to access psch related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicpschAPI struct {
	b Backend
}

// NewPublicpschAPI creates a new psch protocol API.
func NewPublicpschAPI(b Backend) *PublicpschAPI {
	return &PublicpschAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicpschAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicpschAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicpschAPI) Syncing() (interface{}, error) {
...
// Sign calculates an psch ECDSA signature for:
// keccack256("\x19psch Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19psch Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid psch signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and psch.
...
// PublicBlockChainAPI provides an API to access the psch blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new psch blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current psch chain config.
...
// PublicDebugAPI is the collection of psch APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of psch APIs exposed over the private
// debugging endpoint.
...
// Version returns the current psch protocol version.
