...
import (
	...
	"github.com/petercastron/PSCH/les/utils"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // psch protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local psch node is running in archive mode, advertise ourselves we have
...
// participating in the Light psch sub-protocol.
...
// for us(server) but for other protocols(e.g. psch) upon the devp2p they
// may be useful.
