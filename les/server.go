...
type LesServer struct {
	...
	archiveMode bool // Flag whether the psch node runs in archive mode.
...
func NewLesServer(node *node.Node, e ethBackend, config *ethconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "psch/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "psch light client service")
