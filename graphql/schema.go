...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte psch address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an psch account at a particular block.
    type Account {
    ...
    # Transaction is an psch transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an psch account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an psch account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an psch block by number or by hash. If neither is
