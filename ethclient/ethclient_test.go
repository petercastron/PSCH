...
// Verify that Client implements the psch interfaces.
var (
	_ = psch.ChainReader(&Client{})
	_ = psch.TransactionReader(&Client{})
	_ = psch.ChainStateReader(&Client{})
	_ = psch.ChainSyncReader(&Client{})
	_ = psch.ContractCaller(&Client{})
	_ = psch.GasEstimator(&Client{})
	_ = psch.GasPricer(&Client{})
	_ = psch.LogFilterer(&Client{})
	_ = psch.PendingStateReader(&Client{})
	// _ = psch.PendingStateEventer(&Client{})
	_ = psch.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new psch service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == psch.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != psch.NotFound {
		t.Fatal("error should be psch.NotFound")
	}
}
