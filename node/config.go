...
import (
	...
	"github.com/petercastron/PSCH/accounts/external"
	"github.com/petercastron/PSCH/accounts/keystore"
	"github.com/petercastron/PSCH/accounts/scwallet"
	"github.com/petercastron/PSCH/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of psch is "psch". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "psch", keep that.
	if name == "psch" || name == "psch-testnet" {
		name = "psch"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "psch" instances.
var isOldpschResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by psch 1.4 are used if they exist.
	if warn, isOld := isOldpschResource[path]; isOld {
		oldpath := ""
		if c.name() == "psch" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldpschResourceWarning, "Using deprecated resource file %s, please move this file to the 'psch' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "psch-keystore")
		ephemeral = keydir
	}
