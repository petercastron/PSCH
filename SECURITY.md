# Security Policy

## Supported Versions

Please see [Releases](https://github.com/petercastron/PSCH/releases). We recommend using the [most recently released version](https://github.com/petercastron/PSCH/releases/latest).

## Audit reports

Audit reports are published in the `docs` folder: https://github.com/petercastron/PSCH/tree/master/docs/audits 

| Scope | Date | Report Link |
| ------- | ------- | ----------- |
| `psch` | 20170425 | [pdf](https://github.com/petercastron/PSCH/blob/master/docs/audits/2017-04-25_psch-audit_Truesec.pdf) |
| `clef` | 20180914 | [pdf](https://github.com/petercastron/PSCH/blob/master/docs/audits/2018-09-14_Clef-audit_NCC.pdf) |

## Reporting a Vulnerability

**Please do not file a public ticket** mentioning the vulnerability.

To find out how to disclose a vulnerability in psch visit [https://bounty.psch.org](https://bounty.psch.org) or email bounty@psch.org. Please read the [disclosure page](https://github.com/petercastron/PSCH/security/advisories?state=published) for more information about publically disclosed security vulnerabilities.

Use the built-in `psch version-check` feature to check whether the software is affected by any known vulnerability. This command will fetch the latest [`vulnerabilities.json`](https://psch.org/docs/vulnerabilities/vulnerabilities.json) file which contains known security vulnerabilities concerning `psch`, and cross-check the data against its own version number.

The following key may be used to communicate sensitive information to developers.

Fingerprint: `AE96 ED96 9E47 9B00 84F3 E17F E88D 3334 FA5F 6A0A`

```
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1


=r6KK
-----END PGP PUBLIC KEY BLOCK-----
```
